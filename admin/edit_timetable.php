...
$active_ay_id = $timetable['academic_year_id'];
...
<div class="flex justify-between items-center mb-6">
    <h1 class="text-2xl font-semibold text-gray-900">Edit Timetable</h1>
        ...
        <div class="space-y-6">
            <div>
                ...
                <select id="classroom_id" name="classroom_id" required class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md border shadow-sm">
                    ...
                    <?php foreach ($classrooms as $c): ?>
                        <option value="<?php echo $c['id']; ?>" <?php echo $c['id'] == $timetable['classroom_id'] ? 'selected' : ''; ?>>
            ...
            <div>
                ...
                <select id="semester" name="semester" required class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md border shadow-sm">
                    ...
                    <option value="first" <?php echo $timetable['semester'] === 'first' ? 'selected' : ''; ?>>First Semester</option>
                    <option value="second" <?php echo $timetable['semester'] === 'second' ? 'selected' : ''; ?>>Second Semester</option>
                </select>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700">Timetable PDF File (Optional)</label>
                <p class="text-xs text-gray-500 mb-2">Leave empty to keep the existing timetable file: <a href="../../<?php echo htmlspecialchars($timetable['file_path']); ?>" target="_blank" class="text-blue-600 underline">View Current</a></p>
                    ...
                    <div class="space-y-1 text-center">
                        ...
                        <div class="flex text-sm text-gray-600 justify-center">
                            <label for="file_upload" class="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-blue-500">
                                ...
                                <input id="file_upload" name="timetable_file" type="file" accept="application/pdf,image/jpeg,image/png" class="sr-only">
                            </label>
                        </div>
...
document.getElementById('editForm').addEventListener('submit', function(e) {
    ...
    fetch('../ajax/update_timetable.php', {
        method: 'POST',
        body: formData
    })
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            alertContainer.innerHTML = 'Timetable updated successfully! Redirecting...';
            setTimeout(() => {
