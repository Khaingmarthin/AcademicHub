...
$stmt = $pdo->prepare("SELECT id, year_name as name, status, start_date, end_date, created_at, updated_at FROM academic_years WHERE id = :id");
$stmt->execute(['id' => $id]);
$ay = $stmt->fetch();
...
document.getElementById('ayForm').addEventListener('submit', function(e) {
    ...
    fetch('../ajax/update_academic_year.php', {
        method: 'POST',
        body: formData
    })
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            alertContainer.innerHTML = 'Academic year updated successfully!';
        } else {
