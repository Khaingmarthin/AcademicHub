...
if ($ay_id <= 0 || $major_id <= 0 || $ay_level_id <= 0 || empty($section_input)) {
...
try {
    ...
    // Check for duplicate within the same academic year
    $stmt = $pdo->prepare("SELECT id FROM classrooms WHERE classroom_name = :name AND academic_year_id = :ay_id");
    $stmt->execute(['name' => $classroom_name, 'ay_id' => $ay_id]);
    if ($stmt->fetch()) {
        echo json_encode(['success' => false, 'message' => "Classroom '{$classroom_name}' already exists in this academic year."]);
        exit;
    }

    $stmt = $pdo->prepare("INSERT INTO classrooms (academic_year_id, academic_year_level_id, major_id, section, classroom_name, status) VALUES (:ay, :ay_level, :major, :sec, :name, :status)");
    $stmt->execute([
        'ay' => $ay_id,
        'ay_level' => $ay_level_id,
        'major' => $major_id,
        'sec' => $section_input, // store 'None' or 'A', etc.
        'name' => $classroom_name,
        'status' => $status
    ]);
